...
RSpec.describe Cart do
  describe 'Instance Methods' do
    ...
    it '.items' do
      expect(@cart.items.keys).to eq([@ogre, @giant, @dog])
    end

    it '.grand_total' do
      expect(@cart.grand_total).to eq(310)
    end

    it '.count_of()' do
      expect(@cart.count_of(@ogre.id)).to eq(1)
      expect(@cart.count_of(@giant.id)).to eq(2)
      expect(@cart.count_of(@dog.id)).to eq(5)
    end
    ...
    it '.limit_reached?()' do
      ...
      expect(@cart.limit_reached?(@dog.id)).to eq(true)
    end
    ...
    it ".discount_subtotal" do
      expect(@cart.discount_subtotal(@dog)).to eq(190)
    end
  end
end